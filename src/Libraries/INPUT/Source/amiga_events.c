...
#ifdef USE_SDL
static uchar sdlKeyCodeToSSHOCKkeyCode(SDLKey kc) {
...
#else
static uchar amigaRawKeyCodeToSSHOCKkeyCode(UWORD kc, UBYTE ascii)
{
    switch (ascii)
    {
        case 'a':
    ...
    switch (kc)
    {
        case 0x3c:  // SDLK_KP_DECIMAL:
            return 0x41; //  kVK_ANSI_KeypadDecimal   = 0x41,
        case 0x5d:  // SDLK_KP_MULTIPLY:
            return 0x43; //  kVK_ANSI_KeypadMultiply = 0x43,
        case 0x5e:  // SDLK_KP_PLUS:
            return 0x45; //  kVK_ANSI_KeypadPlus = 0x45,
        /*case SDLK_KP_CLEAR:
            return 0x47; //  kVK_ANSI_KeypadClear = 0x47,*/
        case 0x5c:  // SDLK_KP_DIVIDE:
            return 0x4B; //  kVK_ANSI_KeypadDivide = 0x4B,
        case 0x43:  // SDLK_KP_ENTER:
            return 0x4C; //  kVK_ANSI_KeypadEnter   = 0x4C, aka _ENTER2_
        case 0x4a:  // SDLK_KP_MINUS:
            return 0x4E; //  kVK_ANSI_KeypadMinus   = 0x4E,
        /*case SDLK_KP_EQUALS:
            return 0x51; //  kVK_ANSI_KeypadEquals = 0x51,*/
        case 0x0f:  // SDLK_KP_0:
        ...
        // keycodes for keys that are independent of keyboard layout
        case 0x44:  // SDLK_RETURN:
            return 0x24; //  kVK_Return  = 0x24,
        case 0x42:  // SDLK_TAB:
            return 0x30; //  kVK_Tab     = 0x30,
        case 0x40:  // SDLK_SPACE:
            return 0x31; //  kVK_Space   = 0x31,
        case 0x46:  // SDLK_DELETE:
            return 0x33; //  kVK_Delete  = 0x33,
        case 0x41:  // SDLK_BACKSPACE:
            return 0x33; //  kVK_Delete  = 0x33,
        case 0x45:  // SDLK_ESCAPE:
            return 0x35; //  kVK_Escape  = 0x35,
        /*case SDLK_F17:
            return 0x40; //  kVK_F17 = 0x40,
        case SDLK_VOLUMEUP:
...
void SetMouseXY(int mx, int my)
{
    ...
    SDL_ShowCursor((!focus || !MouseCaptured) ? SDL_ENABLE : SDL_DISABLE);
#else
    float scaleX = (float)gPhysicalWidth / gLogicalWidth;
    float scaleY = (float)gPhysicalHeight / gLogicalHeight;
...
static bool saved_rel_mouse = FALSE;

// same codes as returned by amigaRawKeyCodeToSSHOCKkeyCode and sdlKeyCodeToSSHOCKkeyCode functions
uchar Ascii2Code[95] = {
    0x31, // space
    0x12, // !
    0x27, // "
...
void pump_events(void) {
    ...
    while (SDL_PollEvent(&ev))
    {
        switch (ev.type)
        {
            ...
            case SDL_KEYDOWN:
            {
                if (isprint((char)ev.key.keysym.sym))
                {
                    char ascii = (char)ev.key.keysym.sym & 0x7F;
                    ...
                    int c = Ascii2Code[ascii - 32];

                    keyEvent.code = c;
                    keyEvent.ascii = ascii;

                    keyEvent.state = ev.type == SDL_KEYDOWN ? KBS_DOWN : KBS_UP;
                    addKBevent(&keyEvent);

                    if (keyEvent.state == KBS_DOWN)
                    {
    ...
    while ((msg = (struct IntuiMessage *)GetMsg(pMainWindow->UserPort)))
    {
        ...
        switch (msg->Class)
        {
            case IDCMP_CLOSEWINDOW:
                // a bit hacky at this place, but this would allow exiting the game via the window's close button
                exit(0); // TODO: I guess there is a better way.

                break;

            case IDCMP_REFRESHWINDOW:
                RefreshWindowFrame(pMainWindow);
            ...
            case IDCMP_MOUSEBUTTONS:
                ...
                if (mouseEvent.type != 0)
                {
                    ...
                    mouseEvent.y = MouseY;
                    mouseEvent.timestamp = mouse_get_time();
                    mouseEvent.modifiers = (shiftKeysPressed ? 1 : 0);
                    addMouseEvent(&mouseEvent);
                }
            ...
            case IDCMP_MOUSEMOVE:
                ...
                {
                    lastRelMouseX = msg->MouseX;
                    lastRelMouseY = msg->MouseY;

                    SetMouseXY(MouseX + lastRelMouseX, MouseY + lastRelMouseY);
                }
                else*/
                {
                    lastRelMouseX = 0;
                    ...
                    SetMouseXY(pMainWindow->MouseX - pMainWindow->BorderLeft, pMainWindow->MouseY - pMainWindow->BorderTop);
                }
            ...
            case IDCMP_RAWKEY:
                ...
                pEventPtr = msg->IAddress;
                inputEvent.ie_EventAddress = *pEventPtr;

                if (MapRawKey(&inputEvent, keyBuffer, 8, NULL) == 1)
                {
                    ascii = keyBuffer[0];
                }

                if (!(msg->Code & 0x80) && isprint(ascii))
                {
                    kbs_event keyEvent = {0};
                    keyEvent.modifiers = 0;
                ...
                else
                {
                    ...
                    uchar c = amigaRawKeyCodeToSSHOCKkeyCode(maskedCode, ascii);
                    if (c != KBC_NONE)
                    {
                        kbs_event keyEvent = {0};
                        keyEvent.code = c;
                        ...
                        {
                            // use these invented "ascii" codes for hotkey system
                            // see AmigaSrc/Prefs.c and also Amiga OS dev docs for these hardcoded values
                            switch (maskedCode)
                            {
                                case 0x50:  // SDLK_F1:
                                    keyEvent.ascii = 128 + 0;
                        ...
                        UWORD mod = msg->Qualifier;
                        if (mod & (IEQUALIFIER_LSHIFT | IEQUALIFIER_RSHIFT))
                        {
                            keyEvent.modifiers |= KB_MOD_SHIFT;
                        }
                        if (mod & IEQUALIFIER_CONTROL)
                        {
                            keyEvent.modifiers |= KB_MOD_CTRL;
                        }
                        if (mod & (IEQUALIFIER_LALT | IEQUALIFIER_RALT))
                        {
                            keyEvent.modifiers |= KB_MOD_ALT;
                        }
                        ...
                        else
                        {
                            ...
                            // handle non-printable or ctrl'd or alt'd keys here
                            if (ascii < 32 || ascii > 126 || (mod & IEQUALIFIER_CONTROL) || (mod & (IEQUALIFIER_LALT | IEQUALIFIER_RALT)))
                            {
                                keyEvent.state = KBS_DOWN;
                                addKBevent(&keyEvent);
                ...
                if (!(msg->Code & 0x80) && (msg->Qualifier & IEQUALIFIER_LSHIFT == IEQUALIFIER_LSHIFT || msg->Qualifier & IEQUALIFIER_RSHIFT == IEQUALIFIER_RSHIFT))
                {
                    shiftKeysPressed = TRUE;
                }

                // hack to allow pressing shift after move key
                // sets all current shock states in array to shifted or non-shifted
                if (msg->Code == 0x60 || msg->Code == 0x61 || msg->Code == (0x60 | 0x80) || msg->Code == (0x61 | 0x80))
                {
                    for (int i = 0; i < 256; i++)
                    {
                        if (sshockKeyStates[i])
            ...
            case IDCMP_ACTIVEWINDOW:
                SetRelativeMouseMode(saved_rel_mouse);
                if (saved_rel_mouse)
                {
                    // throw away this first relative mouse reading
                    int mvelx, mvely;
            ...
            case IDCMP_INACTIVEWINDOW:
                saved_rel_mouse = GetRelativeMouseMode();
