...
void zoom_rect(LGRect *start, LGRect *end)
{
  ...
  ZoomStart = *start;
  ZoomEnd = *end;
  ZoomTicks = GetMilliseconds();
}

//called just before and after ScreenDraw() in mainloop()
void ZoomDrawProc(int erase)
{
  ...
  if (!erase)
  {
    ZoomI = GetMilliseconds() - ZoomTicks;
...
//gamma param not used here; see SetPalette() in Shock.c
