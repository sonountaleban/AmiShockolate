    ...
    if (music_on) {
        ...
        if (global_fullmap->cyber) {
            ...
            if (play_me != cyber_play) // If music needs to be changed, then
            {
                ...
                AmigaTuneStartCurrentTheme(); // start playing the new tune.
            } else
        ...
        if (gReadyToQueue) {
            ...
            {
                AmigaTuneQueueTune(pid); // Queue it up.
...
void load_score_guts(uint8_t score_play) {
    ...
    rv = AmigaTuneLoadTheme(base, score_play);
...
errtype music_init() {
    ...
    */
    if (gShockPrefs.soBackMusic) {
        //	if (music_card)
        //	{
        if (AmigaTuneInit() == 0) // If no error, go ahead and start up.
        {
